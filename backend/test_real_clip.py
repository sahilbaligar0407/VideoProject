#!/usr/bin/env python3
"""
Test real clip caption burning end-to-end to identify the issue.
"""
...
async def test_real_clip():
    """Test caption burning on a real clip to identify the issue"""
    print("🔍 REAL CLIP CAPTION BURNING AUDIT")
    print("=" * 60)
    ...
    # Check for captioned version
    captioned_clip_path = original_clip_path.replace('.mp4', '_captioned.mp4')
    if os.path.exists(captioned_clip_path):
        captioned_size = os.path.getsize(captioned_clip_path)
        print(f"   Captioned clip: {os.path.abspath(captioned_clip_path)} ({captioned_size} bytes)")
    else:
        print(f"   Captioned clip: NOT FOUND")
    ...
    # C) Test caption burning on the real clip
    print("\n🔥 C) Testing Caption Burning on Real Clip")
    
    if not os.path.exists(ass_path):
        print(f"   ❌ ASS file not found, cannot test burning")
        return
    ...
    try:
        ...
        result = await processor._burn_ass_captions(original_clip_path, ass_path)
    ...
    if os.path.exists(result) and result != original_clip_path:
        ...
        if os.path.exists(watermark_frame):
            ...
            watermark_hash = calculate_sha1(watermark_frame)
            print(f"   Watermark check frame: {watermark_frame} ({watermark_size} bytes, SHA1: {watermark_hash})")
